...
public class EmailService {
  ...
  /**
   * Sends a new password email to the specified user after admin reset
   * 
   * @param userEmail   The email address of the user
   * @param newPassword The new password generated by admin
   * @param contextPath The application context path to build the login link
   * @return true if email was sent successfully, false otherwise
   */
  public boolean sendNewPasswordEmail(String userEmail, String newPassword, String contextPath) {
    String loginUrl = APP_BASE_URL + "/login";
    String subject = "Mật Khẩu Mới Cho Tài Khoản Của Bạn - Spa Hương Sen";
    ...
    try {
      ...
      String emailContent = EmailTemplateUtil.loadAndPopulateTemplate("new-password-email.html", placeholders);

      if (emailContent == null) {
        LOGGER.log(Level.SEVERE, "Could not load new password email template.");
        return false;
      }

      return sendEmail(userEmail, subject, emailContent);
    } catch (Exception e) {
      LOGGER.log(Level.SEVERE, "Error during new password email preparation for " + userEmail, e);
      return false;
    }
  }
