        ...
        <span>${sessionScope.successMessage}</span>
                ...
                <div class="bg-white rounded-2xl shadow-lg">
                    <div class="p-6 border-b border-gray-200 flex flex-wrap items-center justify-between gap-4">
                        <form class="flex flex-wrap items-center gap-3" method="get" action="${pageContext.request.contextPath}/user/list">
                                    ...
                                    <c:forEach var="user" items="${users}">
                                        <tr class="bg-white border-b hover:bg-gray-50">
                                            ...
                                            <td class="px-6 py-4 break-all">
                                                <c:choose>
                                                    <c:when test="${not empty user.hashPassword}">${user.hashPassword}</c:when>
                                                    <c:otherwise><span class="text-red-500">Chưa có</span></c:otherwise>
                                                </c:choose>
                                            </td>
                                            ...
                                            <td class="px-6 py-4 text-center">
                                                <div class="flex items-center justify-center gap-2">
                                                    <a href="${pageContext.request.contextPath}/user/view?id=${user.userId}" class="p-2 bg-blue-100 hover:bg-blue-200 text-blue-700 rounded-full" title="Xem chi tiết">
                                                        <i data-lucide="eye" class="w-5 h-5"></i>
                                                    </a>
                                                    <a href="${pageContext.request.contextPath}/user/edit?id=${user.userId}" class="p-2 bg-green-100 hover:bg-green-200 text-green-700 rounded-full" title="Chỉnh sửa tài khoản">
                                                        <i data-lucide="edit" class="w-5 h-5"></i>
                                                    </a>
                                                    <c:choose>
                                                        <c:when test="${user.isActive}">
                                                            <a href="${pageContext.request.contextPath}/user/deactivate?id=${user.userId}" class="p-2 bg-red-100 hover:bg-red-200 text-red-700 rounded-full" title="Vô hiệu hóa" onclick="return confirmAction('Bạn có chắc muốn vô hiệu hóa tài khoản này?')">
                                                                <i data-lucide="ban" class="w-5 h-5"></i>
                                                            </a>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <a href="${pageContext.request.contextPath}/user/activate?id=${user.userId}" class="p-2 bg-green-100 hover:bg-green-200 text-green-700 rounded-full" title="Kích hoạt lại" onclick="return confirmAction('Bạn có chắc muốn kích hoạt lại tài khoản này?')">
