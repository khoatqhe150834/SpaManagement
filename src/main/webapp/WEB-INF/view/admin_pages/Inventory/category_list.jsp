    ...
    <main class="w-full md:w-[calc(100%-256px)] md:ml-64 min-h-screen transition-all">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-20">
            <div class="flex flex-wrap items-center justify-between gap-4 mb-8">
                <div>
                    <h1 class="text-3xl font-serif text-spa-dark font-bold">Danh sách loại vật tư</h1>
                ...
                <a href="category/create" class="inline-flex items-center gap-2 h-10 px-4 bg-primary text-white font-semibold rounded-lg hover:bg-primary-dark transition-colors">
            ...
            <div class="bg-white rounded-2xl shadow-lg">
                <div class="p-6 border-b border-gray-200 flex flex-wrap items-center justify-between gap-4">
                    <form class="flex flex-wrap items-center gap-3" method="get" action="">
                        ...
                        <input type="text" name="search" value="${param.search}" placeholder="Tìm kiếm loại vật tư..." class="h-10 px-4 text-base placeholder-gray-600 border rounded-lg focus:shadow-outline w-64" />
                        ...
                        <table class="w-full text-sm text-left text-gray-500">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3">Tên loại</th>
                            ...
                            <tbody>
                                <c:forEach var="cat" items="${categories}">
                                    <tr class="bg-white border-b hover:bg-gray-50">
                                        <td class="px-6 py-4 font-medium text-gray-900">${cat.inventoryCategoryId}</td>
                                        <td class="px-6 py-4 max-w-xs truncate" title="${cat.name}">${cat.name}</td>
                                        <td class="px-6 py-4 max-w-xs truncate" title="${cat.description}">${cat.description}</td>
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            <div class="flex items-center justify-center gap-2">
                                                <a href="category/edit?id=${cat.inventoryCategoryId}" class="p-2 bg-green-100 hover:bg-green-200 text-green-700 rounded-full" title="Sửa loại vật tư">
                                                    <i data-lucide="edit" class="w-5 h-5"></i>
                                                </a>
                                                <form action="category/delete" method="post" style="display:inline">
                                                    <input type="hidden" name="id" value="${cat.inventoryCategoryId}" />
                                                    <button type="submit" onclick="return confirm('Xác nhận vô hiệu hóa loại vật tư?')" class="p-2 bg-red-100 hover:bg-red-200 text-red-700 rounded-full border-none cursor-pointer" title="Xóa loại vật tư">
