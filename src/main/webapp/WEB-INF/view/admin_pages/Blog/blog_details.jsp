<%-- 
    ...
    Created on : Jun 18, 2025, 2:22:55 PM
    Author     : ADMIN
--%>
                    ...
                    <div class="card p-0 radius-12 overflow-hidden">
                        <div class="card-body p-0">
                            <div style="aspect-ratio: 16/9; width: 100%; overflow: hidden;">
                                <img src="${pageContext.request.contextPath}/${empty blog.featureImageUrl ? 'assets/admin/images/blog/blog-details.png' : blog.featureImageUrl}" alt="" class="w-100 h-100 object-fit-cover" style="object-fit: cover; width: 100%; height: 100%;" />
                            </div>
                            <div class="p-32">
                                <div class="d-flex align-items-center gap-16 justify-content-between flex-wrap mb-24">
                                    <div class="d-flex align-items-center gap-8">
                                        <img src="${pageContext.request.contextPath}/assets/admin/images/user-list/user-list1.png" alt="" class="w-48-px h-48-px rounded-circle object-fit-cover">
                                        ...
                                        <c:forEach var="comment" items="${comments}">
                                            <div class="comment-list__item">
                                                <div class="d-flex align-items-start gap-16">
                                                    ...
                                                    <div class="flex-grow-1 border-bottom pb-40 mb-40 border-dashed d-flex align-items-center justify-content-between">
                                                        <div>
                                                            <h6 class="text-lg mb-4 mb-lg-0">${empty comment.customerName ? comment.guestName : comment.customerName}</h6>
                                                            <span class="text-neutral-500 text-sm">
                                                                <fmt:formatDate value="${comment.createdAtDate}" pattern="MMM dd, yyyy 'at' HH:mm"/>
                                                            </span>
                                                            <p class="text-neutral-600 text-md my-16">${comment.commentText}</p>
                                                        </div>
                                                        <c:if test="${sessionScope.user != null && sessionScope.user.roleId == 6}">
                                                            <form method="post" action="${pageContext.request.contextPath}/blog" style="display:inline; margin-left: 12px;">
                                                                <input type="hidden" name="action" value="rejectComment" />
                                                                <input type="hidden" name="commentId" value="${comment.commentId}" />
                                                                <input type="hidden" name="slug" value="${blog.slug}" />
                                                                <button type="submit" class="btn btn-sm btn-danger radius-8">Reject Comment</button>
                                                            </form>
                                                        </c:if>
                                                    </div>
                                                </div>
                            ...
                            <div class="card-body d-flex flex-column gap-24 p-24">
                                <c:forEach var="recent" items="${recentBlogs}">
                                    <div class="d-flex flex-wrap mb-16">
                                        <a href="${pageContext.request.contextPath}/blog?slug=${recent.slug}" class="blog__thumb w-100 radius-12 overflow-hidden" style="aspect-ratio: 16/9; display: block;">
                                            <img src="${pageContext.request.contextPath}/${empty recent.featureImageUrl ? 'assets/admin/images/blog/blog5.png' : recent.featureImageUrl}" alt="" class="w-100 h-100 object-fit-cover" style="object-fit: cover; width: 100%; height: 100%;" />
                                        </a>
                                        <div class="blog__content">
                                            <h6 class="mb-8">
                                                <a href="${pageContext.request.contextPath}/blog?slug=${recent.slug}" class="text-line-2 text-hover-primary-600 text-md transition-2">${recent.title}</a>
                                            </h6>
                                            <p class="text-line-2 text-sm text-neutral-500 mb-0">${recent.summary}</p>
                                        </div>
                                    </div>
                                </c:forEach>
                            </div>
