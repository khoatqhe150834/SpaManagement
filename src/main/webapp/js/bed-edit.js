/**
 * Bed Edit Form JavaScript
 * Handles validation and form submission for editing existing beds
 */

document.addEventListener('DOMContentLoaded', function() {
    // Form validation
    document.getElementById('editBedForm').addEventListener('submit', function(e) {
        let isValid = true;
    ...
    const currentBedId = document.querySelector('input[name="bedId"]').value;
    ...
    document.getElementById('name').addEventListener('input', function() {
        ...
        nameValidationTimeout = setTimeout(() => {
            validateBedNameAjax(name, nameInput, nameError, roomId, currentBedId);
        }, 300);
    });
    ...
    function validateBedNameAjax(name, inputElement, errorElement, roomId, bedId) {
        ...
        // Make AJAX request (exclude current bed)
        fetch('/' + contextPath + '/api/validate/bed/name/' + bedId, {
            method: 'POST',
            headers: {
